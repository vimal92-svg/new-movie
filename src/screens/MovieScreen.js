...
import { StyleSheet, Text, View,ScrollView,Dimensions,Image,TouchableOpacity, Linking,FlatList,Share } from 'react-native';
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.moviePosterImageContainer}>
        <Image style={styles.moviePosterImage} rewsizeMode="cover"source={{uri:getPoster(movie?.backdrop_path)}}/>
        </View>
        ...
        <View style={styles.movieTitleContainer}>
          <Text style={styles.movieTitle} number of lines ={2}>{movie?.original_title}</Text>
           <View style={styles.row}>
           <Ionicons name="heart" size={22} color={COLORS.HEART} />
             <Text style={styles.ratingText}>{movie?.vote_average}</Text>
             </View>
        </View>
        <Text style={styles.genreText}>{movie?.genres?.map(genre => genre?.name)?.join(",")}|{" "}
        ...
        <View>
          ...
          renderItem={({item})=>( <CastCard originalName={item?.name} characterName={item?.character}
